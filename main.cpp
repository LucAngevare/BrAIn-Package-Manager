...
int main() {
    uWS::App().get("/get/:name", [](auto *res, auto *req) {
        ...
        res->onData([contentLength, res, dataStr, buffer, formdata, ID](std::string_view data, bool last) mutable {
            ...
            if (last) {
                try {
                    ...
                    if (std::regex_search(dataString, match, std::regex("-++.+\\nContent-Disposition: form-data; name=\"(\\w+|\\ +)+\"; filename=\"(.+)\"\\nContent-Type: (\\w+\\/\\w+\\n)\\n((.|\\n)+)\\n-+.+--.+|\\n+"))) {
                        ...
                        fs::create_directory((fs::path)"./packages", ID);

                        std::ofstream fileToWrite(path + formdata["FileName"].dump());
                        if (fileToWrite.is_open())
                            fileToWrite << formdata["FileData"];
                        fileToWrite.close();
                    } else if (std::regex_search(buffer, match, std::regex("Content-Disposition:\\ form-data;\\ name=\\\"(\\w+|\\ +)+\\\";\\nContent-Type: (\\w+\\/\\w+\\n)\\n((.|\\n)+)\\n-+.+--.+|\\n+"))) {
        ...
        res->end("does this even fucking work?");
        /* res->cork([]() {
            res->end()
        }) */
